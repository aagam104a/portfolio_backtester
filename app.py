...
def asset_allocation_section(settings: dict) -> tuple[dict[str, float], dict[str, float]]:
    ...
    with st.container():
        ...
        for ticker in selected_tickers:
            ...
            default_alloc = float(
                preset_alloc.get(ticker, round(100.0 / len(selected_tickers), 1))
            )
...
def backtest_section(settings: dict, allocations: dict, expense_ratios: dict) -> dict | None:
    ...
    with c3:
        ...
        _metric_card(
            "XIRR",
            _fmt_pct(xirr_val) if not np.isnan(xirr_val) else "N/A",
            "Most accurate SIP metric",
            "good" if (not np.isnan(xirr_val) and xirr_val > 0) else "warn",
        )
    ...
    with c4:
        ...
        _metric_card(
            "Max Drawdown",
            _fmt_pct(-dd),
            f"Recovery: {m['recovery_months'] or 'Not yet'} months",
            "bad",
        )
        _metric_card(
            "Sharpe Ratio",
            f"{m['sharpe_ratio']:.2f}" if not np.isnan(m["sharpe_ratio"]) else "N/A",
        )
    ...
    with c6:
        _metric_card(
            "Best Month",
            _fmt_pct(m["best_month_return"]),
            str(m["best_month_date"])[:7] if m["best_month_date"] else "",
            "good",
        )
    with c7:
        _metric_card(
            "Worst Month",
            _fmt_pct(m["worst_month_return"]),
            str(m["worst_month_date"])[:7] if m["worst_month_date"] else "",
            "bad",
        )
    with c8:
    ...
    with tab3:
        ...
        with col_a:
            ...
            st.plotly_chart(
                fig,
                use_container_width=True,
                key="allocation_pie_tab_chart",
            )

        with col_b:
            fig = asset_contribution_bar(
                result.asset_contributions,
                {t: _ticker_label(t) for t in allocations},
            )
            st.plotly_chart(
                fig,
                use_container_width=True,
...
def crash_simulation_section(settings: dict) -> None:
    ...
    with col1:
        ...
        recovery_cagr = (
            st.number_input("Recovery CAGR assumption (%)", 5.0, 30.0, 12.0, 1.0) / 100
        )
    ...
    if run_crash:
        ...
        m2.metric(
            "Post-Crash Value",
            _fmt_inr(crash_result.post_crash_value),
            delta=f"-{_fmt_inr(crash_result.drawdown_amount)}",
            delta_color="inverse",
        )
        ...
        if crash_result.months_to_recover is not None:
            m4.metric(
                "Months to Recover",
                crash_result.months_to_recover,
    ...
    for crash_name, crash_info in HISTORICAL_CRASHES.items():
        with st.expander(f"📉 {crash_name}"):
            ...
            if replay["data_available"]:
                ...
                rc4.metric(
                    "Recovery",
                    f"{replay['recovery_months']} months"
                    if replay["recovery_months"]
                    else "Not yet",
                )
...
def go_lines_comparison(comp_results: dict) -> object:
    ...
    fig = go.Figure()
    colors = ["#4f8df5", "#f5a44f", "#5ec26b", "#e05252", "#a855f7", "#ec4899"]
...
def strategy_comparison_section(settings: dict) -> None:
    ...
    run_compare = st.button(
        "🔁 Compare All Example Strategies",
        type="secondary",
        use_container_width=True,
    )
    ...
    if run_compare:
        ...
        for i, strategy in enumerate(strategies):
            ...
            pct = int((i + 1) / len(strategies) * 100)
            pct = max(0, min(100, pct))
            comparison_progress.progress(pct)
